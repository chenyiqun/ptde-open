...
class NSNCQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if self.args.name == 'snc_env=8_adam_td_lambda':  # snc
            SN = []
            C = []
        ...
        for t in range(batch.max_seq_length):
            ...
            if self.args.name == 'snc_env=8_adam_td_lambda':  # snc
                ...
                # out sni ci
                agent_outs, sn_i, c_i = self.mac.forward(batch, t=t, test_mode=False)  # mac
        ...
        # kl-loss
        kl_loss = 0
        a = batch.groups['agents']
        if self.args.name == 'snc_env=8_adam_td_lambda':
            for step in range(batch.max_seq_length):
                cur_kl_loss = []
                for i in range(a):
                    for j in range(a):
                        cur_kl_loss.append(kl_divergence(C[step][i], C[step][j]).sum(dim=-1))
                cur_kl = th.stack(cur_kl_loss, dim=1).view(-1, a, a)
                cur_kl = cur_kl.permute(0, 2, 1)  # 128* 3 * 3, batch_size * agents * agents
                cur_sn = SN[step]  # 128* 3 * 3, batch_size * agents * agents
                cur_kl_loss_final = th.matmul(cur_sn, cur_kl, out=None).sum()
                kl_loss += cur_kl_loss_final
        kl_loss /= batch.batch_size * batch.max_seq_length * a * a  # batch_size * max_len * agents * agents
        loss += kl_loss
