...
from modules.gire.mha import MHA


class GIREAgent(nn.Module):
    def __init__(self, args):
        super(GIREAgent, self).__init__()
        self.args = args
    ...
    def forward(self, input_hidden_out, z_dot):
        b, a, e = input_hidden_out.size()

        h_oi = F.relu(self.fc1(input_hidden_out.view(-1, e)), inplace=True)

        q = self.fc2(torch.concat([h_oi, z_dot.view(b * a, -1)], dim=-1))
