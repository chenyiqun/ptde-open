...
def run(_run, _config, _log):
    ...
    # if args.learner == 'gire_learner' and not args.mi_loss:
    #     unique_token = "{}__{}".format('womi_'+args.name, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    # elif args.learner == 'herl_learner' and not args.hyper_net:
    #     unique_token = "{}__{}".format('wohyper_' + args.name, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    # else:
...
def evaluate_sequential(args, runner):
    ...
    aver_win = []
    for i in range(10):  # args.test_nepisode
...
def run_sequential(args, logger):
    ...
    if args.checkpoint_path != "":
        ...
        args.checkpoint_path = os.path.join(args.checkpoint_path, args.env_args['map_name'],
                                            'snc_env/seed_{}'.format(args.seed))  # qmix_env gire_z_env snc_env
        ...
        if args.generate_data:
            ...
            for k in range(100):
                ...
                with th.no_grad():
                    ...
                    episode_data = th.cat((episode_obs_input, episode_states), dim=-1).view(-1, (o_dim+a_dim+n_agent)+s_dim).cpu().numpy()
    ...
    # 最后一次没必要保存。。。。
    # plot_win_rates = np.array(plot_win_rates)
    # plot_path = os.path.join("results", "plot_data", args.env_args['map_name'], args.unique_token)
    # os.makedirs(plot_path, exist_ok=True)
    # np.save(os.path.join(plot_path, "win_rates_seed_{}.npy".format(args.seed)), plot_win_rates)
    # logger.console_logger.info("Saving plot data to {}".format(plot_path))
